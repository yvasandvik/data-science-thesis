...
load("~/Documents/NMBU/Semester 12/Data Science Master/data-science-thesis/data_expo/G7_orfs.RData")


G.dataset <- subset(G7.dataset, select=c(Length, Sequence))
...
colnames(Kmer_2) <- paste("k_mer", colnames(Kmer_2), sep = "_")
```
...
colnames(Kmer_3) <- paste("k_mer", colnames(Kmer_3), sep = "_")
```
...
colnames(Kmer_4) <- paste("k_mer", colnames(Kmer_4), sep = "_")
```
...
colnames(Kmer_5) <- paste("k_mer", colnames(Kmer_5), sep = "_")
```
...
colnames(Kmer_6) <- paste("k_mer", colnames(Kmer_6), sep = "_")
```
...
colnames(aa_mer_1) <- paste("aa_mer", colnames(aa_mer_1), sep = "_")
```
...
colnames(aa_mer_2) <- paste("aa_mer", colnames(aa_mer_2), sep = "_")
```
...
colnames(aa_mer_3) <- paste("aa_mer", colnames(aa_mer_3), sep = "_")
```
...
G_features <- bind_cols(G7.dataset, GC_features, Start_features, Kmer_2, Kmer_3, Kmer_4, Kmer_5, Kmer_6, aa_mer_1, aa_mer_2, aa_mer_3, c_weights)
...
G7.features <- G_features

# RData
save(G7.features, file="G7_orf_features.RData")
