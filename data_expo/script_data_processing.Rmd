...
fig_hist <- ggplot(data=G.dataset, aes(Length, colour = Type)) + 
  ...
  ylab('Number of ORFs') + ggtitle('Length distribution of CDS and LORFs') + facet_grid(rows = vars(Type), scales = "free_y")
...
p5 <- sample_n(cat_5, sum(summary_cds$Count[4]), replace=FALSE)
...
bind_rows(p5_extra, p2_extra, p3_extra, p4_extra) %>% 
  mutate(Dataset = 2) -> G.lorfs.selected2
